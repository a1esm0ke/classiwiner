...
wine_data = pd.read_csv(r"C:\Users\diclo\Desktop\ICon-main\winequality.csv")
group_names = ['BAD', 'GOOD']


# Assumendo che `data` sia il tuo DataFrame e 'quality' la variabile dipendente
def plot_regression(data):
    sns.regplot(x='alcohol', y='quality', data=data)
    plt.title('Relazione tra Alcol e Qualità del Vino')
    plt.xlabel('Contenuto Alcolico (%)')
    plt.ylabel('Qualità')
    plt.show()
...
def plot_results(classifier,X_test,y_test, name):
    ...
    cm = confusion_matrix(y_test, pred, labels=group_names)
    ax= plt.subplot()
...
clustering_data = wine_data.copy()
X = wine_data.drop('quality', axis=1)
# Definizione e addestramento del modello KMeans
kmeans = KMeans(n_clusters=5, random_state=42)
kmeans.fit(X_scaled)
wine_data['Cluster'] = kmeans.labels_
# Visualizzazione dei risultati del clustering
sns.scatterplot(x='alcohol', y='quality', hue='Cluster', data=wine_data, palette='viridis')
plt.title('Clustering of Wine Data')
plt.show()
